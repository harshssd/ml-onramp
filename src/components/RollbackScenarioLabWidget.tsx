...
interface RollbackScenarioLabWidgetProps {
  onComplete: () => void;
  config: any;
}

export default function RollbackScenarioLabWidget({ onComplete, config }: RollbackScenarioLabWidgetProps) {
              ...
              <div className={`p-4 rounded-lg ${getColorClasses(currentStepData.color)} border`}>
                ...
                <ul className={`space-y-1 ${themeClasses.text}/80`}>
                  {currentStep.id === 'assess' && (
                    <>
                      <li>• Check error rates and user impact</li>
                      <li>• Review monitoring dashboards</li>
                      <li>• Assess business impact</li>
                    </>
                  )}
                  {currentStep.id === 'prepare' && (
                    <>
                      <li>• Identify target version (v1.0)</li>
                      <li>• Prepare rollback configuration</li>
                      <li>• Notify stakeholders</li>
                    </>
                  )}
                  {currentStep.id === 'execute' && (
                    <>
                      <li>• Deploy v1.0 to production</li>
                      <li>• Update load balancer configuration</li>
                      <li>• Monitor deployment progress</li>
                    </>
                  )}
                  {currentStep.id === 'validate' && (
                    <>
                      <li>• Verify service is responding</li>
                      <li>• Check error rates are normal</li>
                      <li>• Confirm rollback success</li>
                    </>
                  )}
                </ul>
              </div>
